...
#include "NLTeamSubsystem.generated.h"

class AActor;
class ANLPlayerState;
class ANLTeamInfoBase;
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnNLTeamDisplayAssetChangedDelegate, const UNLTeamDisplayAsset*, DisplayAsset);

USTRUCT()
struct FNLTeamTrackingInfo
{
	GENERATED_BODY()

public:
	UPROPERTY()
	TObjectPtr<ANLTeamPublicInfo> PublicInfo = nullptr;

	UPROPERTY()
	TObjectPtr<ANLTeamPrivateInfo> PrivateInfo = nullptr;

	UPROPERTY()
	TObjectPtr<UNLTeamDisplayAsset> DisplayAsset = nullptr;

	UPROPERTY()
	FOnNLTeamDisplayAssetChangedDelegate OnTeamDisplayAssetChanged;

public:
	void SetTeamInfo(ANLTeamInfoBase* Info);
	void RemoveTeamInfo(ANLTeamInfoBase* Info);
};

// Result of comparing the team affiliation for two actors
UENUM(BlueprintType)
enum class ENLTeamComparison : uint8
{
...
UCLASS()
class WOPGAME_API UNLTeamSubsystem : public UWorldSubsystem
{
	GENERATED_BODY()
...
public:
	...
	bool RegisterTeamInfo(ANLTeamInfoBase* TeamInfo);

	// Tries to unregister a team, will return false if it didn't work
	bool UnregisterTeamInfo(ANLTeamInfoBase* TeamInfo);
	...
	const ANLPlayerState* FindPlayerStateFromActor(const AActor* PossibleTeamActor) const;
	...
	ENLTeamComparison CompareTeams(const UObject* A, const UObject* B, int32& TeamIdA, int32& TeamIdB) const;

	// Compare the teams of two actors and returns a value indicating if they are on same teams, different teams, or one/both are invalid
	ENLTeamComparison CompareTeams(const UObject* A, const UObject* B) const;
	...
	UFUNCTION(BlueprintCallable, Category=Teams)
	UNLTeamDisplayAsset* GetTeamDisplayAsset(int32 TeamId, int32 ViewerTeamId);

	// Gets the team display asset for the specified team, from the perspective of the specified team
	// (You have to specify a viewer too, in case the game mode is in a 'local player is always blue team' sort of situation)
	UFUNCTION(BlueprintCallable, Category = Teams)
	UNLTeamDisplayAsset* GetEffectiveTeamDisplayAsset(int32 TeamId, UObject* ViewerTeamAgent);
	...
	void NotifyTeamDisplayAssetModified(UNLTeamDisplayAsset* ModifiedAsset);

	// Register for a team display asset notification for the specified team ID
	FOnNLTeamDisplayAssetChangedDelegate& GetTeamDisplayAssetChangedDelegate(int32 TeamId);

private:
	UPROPERTY()
	TMap<int32, FNLTeamTrackingInfo> TeamMap;
