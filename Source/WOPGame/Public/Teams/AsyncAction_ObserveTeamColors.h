...
class INLTeamAgentInterface;
class UNLTeamDisplayAsset;
struct FFrame;

DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FTeamColorObservedAsyncDelegate, bool, bTeamSet, int32, TeamId, const UNLTeamDisplayAsset*, DisplayAsset);
...
public:
	...
	//  - For anything that can ever belong to a team (implements INLTeamAgentInterface),
...
private:
	void BroadcastChange(int32 NewTeam, const UNLTeamDisplayAsset* DisplayAsset);

	UFUNCTION()
	void OnWatchedAgentChangedTeam(UObject* TeamAgent, int32 OldTeam, int32 NewTeam);

	UFUNCTION()
	void OnDisplayAssetChanged(const UNLTeamDisplayAsset* DisplayAsset);

	TWeakInterfacePtr<INLTeamAgentInterface> TeamInterfacePtr;
