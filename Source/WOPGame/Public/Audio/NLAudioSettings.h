...
#include "NLAudioSettings.generated.h"
...
USTRUCT()
struct WOPGAME_API FNLSubmixEffectChainMap
{
	GENERATED_BODY()
 ...
 * 
 */
UCLASS(config = Game, defaultconfig, meta = (DisplayName = "NLAudioSettings"))
class WOPGAME_API UNLAudioSettings : public UDeveloperSettings
{
	GENERATED_BODY()
...
public:
	...
	TArray<FNLSubmixEffectChainMap> HDRAudioSubmixEffectChain;
	
	/** Submix Processing Chains to achieve low dynamic range audio output */
	UPROPERTY(config, EditAnywhere, Category = EffectSettings)
	TArray<FNLSubmixEffectChainMap> LDRAudioSubmixEffectChain;
