...
#include "NLInputModifiers.generated.h"

struct FInputActionValue;

class FProperty;
class UEnhancedPlayerInput;
class UNLAimSensitivityData;
class UObject;
...
class UNLSettingBasedScalar : public UInputModifier
{
	GENERATED_BODY()
...
/**
 * This is a deadzone input modifier that will have it's thresholds driven by what is in the NL Shared game settings. 
 */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "NL Settings Driven Dead Zone"))
class UNLInputModifierDeadZone : public UInputModifier
{
	GENERATED_BODY()
...
UENUM()
enum class ENLTargetingType : uint8
{
...
/** Applies a scalar modifier based on the current gamepad settings in NL Shared game settings.  */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "NL Gamepad Sensitivity"))
class UNLInputModifierGamepadSensitivity : public UInputModifier
{
	GENERATED_BODY()
public:
	...
	UPROPERTY(EditInstanceOnly, BlueprintReadWrite, Category=Settings, Config)
	ENLTargetingType TargetingType = ENLTargetingType::Normal;

	/** Asset that gives us access to the float scalar value being used for sensitivty */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, meta=(AssetBundles="Client,Server"))
	TObjectPtr<const UNLAimSensitivityData> SensitivityLevelTable;

protected:
	virtual FInputActionValue ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime) override;
};

/** Applies an inversion of axis values based on a setting in the NL Shared game settings */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "NL Aim Inversion Setting"))
class UNLInputModifierAimInversion : public UInputModifier
{
	GENERATED_BODY()
