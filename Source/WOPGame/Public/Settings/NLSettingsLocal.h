...
#include "NLSettingsLocal.generated.h"

enum class ECommonInputType : uint8;
enum class ENLDisplayablePerformanceStat : uint8;
enum class ENLStatDisplayMode : uint8;
...
USTRUCT()
struct FNLScalabilitySnapshot
{
	GENERATED_BODY()

	FNLScalabilitySnapshot();
 ...
 */
UCLASS()
class UNLSettingsLocal : public UGameUserSettings
{
	GENERATED_BODY()
...
public:
	...
	ENLStatDisplayMode GetPerfStatDisplayState(ENLDisplayablePerformanceStat Stat) const;
	
	/** Sets the display mode for the specified performance stat */
	void SetPerfStatDisplayState(ENLDisplayablePerformanceStat Stat, ENLStatDisplayMode DisplayMode);

	/** Fired when the display state for a performance stat has changed, or the settings are applied */
	DECLARE_EVENT(UNLSettingsLocal, FPerfStatSettingsChanged);
...
private:
	...
	UPROPERTY(Config)
	TMap<ENLDisplayablePerformanceStat, ENLStatDisplayMode> DisplayStatList;
...
private:
	...
	void OverrideQualityLevelsToScalabilityMode(const FNLScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels);
...
private:
	...
	FNLScalabilitySnapshot DeviceDefaultScalabilitySettings;
...
private:
	...
	// Audio - Volume
public:
	DECLARE_EVENT_OneParam(UNLSettingsLocal, FAudioDeviceChanged, const FString& /*DeviceId*/);
