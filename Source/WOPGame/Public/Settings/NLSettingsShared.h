...
#include "NLSettingsShared.generated.h"
...
UENUM(BlueprintType)
enum class ENLAllowBackgroundAudioSetting : uint8
{
	Off,
	AllSounds,
...
class UNLLocalPlayer;

/**
 * UNLSettingsShared - The "Shared" settings are stored as part of the USaveGame system, these settings are not machine
 ...
 */
UCLASS()
class UNLSettingsShared : public ULocalPlayerSaveGame
{
	GENERATED_BODY()

public:
	DECLARE_EVENT_OneParam(UNLSettingsShared, FOnSettingChangedEvent, UNLSettingsShared* Settings);
...
public:
	...
	static UNLSettingsShared* CreateTemporarySettings(const UNLLocalPlayer* LocalPlayer);
	
	/** Synchronously loads a settings object, this is not valid to call before login */
	static UNLSettingsShared* LoadOrCreateSettings(const UNLLocalPlayer* LocalPlayer);

	DECLARE_DELEGATE_OneParam(FOnSettingsLoadedEvent, UNLSettingsShared* Settings);

	/** Starts an async load of the settings object, calls Delegate on completion */
	static bool AsyncLoadOrCreateSettings(const UNLLocalPlayer* LocalPlayer, FOnSettingsLoadedEvent Delegate);
...
public:
	UFUNCTION()
	ENLAllowBackgroundAudioSetting GetAllowAudioInBackgroundSetting() const { return AllowAudioInBackground; }
	UFUNCTION()
	void SetAllowAudioInBackgroundSetting(ENLAllowBackgroundAudioSetting NewValue);

	void ApplyBackgroundAudioSettings();

private:
	UPROPERTY()
	ENLAllowBackgroundAudioSetting AllowAudioInBackground = ENLAllowBackgroundAudioSetting::Off;
...
public:
	UFUNCTION()
	ENLGamepadSensitivity GetGamepadLookSensitivityPreset() const { return GamepadLookSensitivityPreset; }
	UFUNCTION()
	void SetLookSensitivityPreset(ENLGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadLookSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	UFUNCTION()
	ENLGamepadSensitivity GetGamepadTargetingSensitivityPreset() const { return GamepadTargetingSensitivityPreset; }
	UFUNCTION()
	void SetGamepadTargetingSensitivityPreset(ENLGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadTargetingSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	void ApplyInputSensitivity();
	
private:
	UPROPERTY()
	ENLGamepadSensitivity GamepadLookSensitivityPreset = ENLGamepadSensitivity::Normal;
	UPROPERTY()
	ENLGamepadSensitivity GamepadTargetingSensitivityPreset = ENLGamepadSensitivity::Normal;
