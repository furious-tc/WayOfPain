...
#include UE_INLINE_GENERATED_CPP_BY_NAME(NLGameplayCueManager)

//////////////////////////////////////////////////////////////////////

enum class ENLEditorLoadMode
{
...
namespace NLGameplayCueManagerCvars
{
	static FAutoConsoleCommand CVarDumpGameplayCues(
		TEXT("NL.DumpGameplayCues"),
		TEXT("Shows all assets that were loaded via NLGameplayCueManager and are currently in memory."),
		FConsoleCommandWithArgsDelegate::CreateStatic(UNLGameplayCueManager::DumpGameplayCues));

	static ENLEditorLoadMode LoadMode = ENLEditorLoadMode::LoadUpfront;
}
...
UNLGameplayCueManager::UNLGameplayCueManager(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

UNLGameplayCueManager* UNLGameplayCueManager::Get()
{
	return Cast<UNLGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
}

void UNLGameplayCueManager::OnCreated()
{
	Super::OnCreated();
...
void UNLGameplayCueManager::LoadAlwaysLoadedCues()
{
	if (ShouldDelayLoadGameplayCues())
	{
		...
		for (const FName& CueTagName : AdditionalAlwaysLoadedCueTags)
		{
			...
			{
				UE_LOG(LogNL, Warning, TEXT("UNLGameplayCueManager::AdditionalAlwaysLoadedCueTags contains invalid tag %s"), *CueTagName.ToString());
			}
		}
	}
}

bool UNLGameplayCueManager::ShouldAsyncLoadRuntimeObjectLibraries() const
{
	switch (NLGameplayCueManagerCvars::LoadMode)
	{
	case ENLEditorLoadMode::LoadUpfront:
		return true;
	case ENLEditorLoadMode::PreloadAsCuesAreReferenced_GameOnly:
#if WITH_EDITOR
		if (GIsEditor)
		{
			return false;
...
#endif
		break;
	case ENLEditorLoadMode::PreloadAsCuesAreReferenced:
		break;
	}
...
bool UNLGameplayCueManager::ShouldSyncLoadMissingGameplayCues() const
{
	return false;
}

bool UNLGameplayCueManager::ShouldAsyncLoadMissingGameplayCues() const
{
	return true;
}

void UNLGameplayCueManager::DumpGameplayCues(const TArray<FString>& Args)
{
	UNLGameplayCueManager* GCM = Cast<UNLGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!GCM)
	{
		UE_LOG(LogNL, Error, TEXT("DumpGameplayCues failed. No UNLGameplayCueManager found."));
		return;
	}

	const bool bIncludeRefs = Args.Contains(TEXT("Refs"));

	UE_LOG(LogNL, Log, TEXT("=========== Dumping Always Loaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->AlwaysLoadedCues)
	{
		UE_LOG(LogNL, Log, TEXT("  %s"), *GetPathNameSafe(CueClass));
	}

	UE_LOG(LogNL, Log, TEXT("=========== Dumping Preloaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->PreloadedCues)
	{
		...
		UE_LOG(LogNL, Log, TEXT("  %s (%d refs)"), *GetPathNameSafe(CueClass), NumRefs);
		if (bIncludeRefs && ReferencerSet)
		{
			for (const FObjectKey& Ref : *ReferencerSet)
			{
				UObject* RefObject = Ref.ResolveObjectPtr();
				UE_LOG(LogNL, Log, TEXT("    ^- %s"), *GetPathNameSafe(RefObject));
			}
		}
	}

	UE_LOG(LogNL, Log, TEXT("=========== Dumping Gameplay Cue Notifies loaded on demand ==========="));
	...
	if (GCM->RuntimeGameplayCueObjectLibrary.CueSet)
	{
		for (const FGameplayCueNotifyData& CueData : GCM->RuntimeGameplayCueObjectLibrary.CueSet->GameplayCueData)
		{
			if (CueData.LoadedGameplayCueClass && !GCM->AlwaysLoadedCues.Contains(CueData.LoadedGameplayCueClass) && !GCM->PreloadedCues.Contains(CueData.LoadedGameplayCueClass))
			{
				...
				UE_LOG(LogNL, Log, TEXT("  %s"), *CueData.LoadedGameplayCueClass->GetPathName());
			}
		}
	}

	UE_LOG(LogNL, Log, TEXT("=========== Gameplay Cue Notify summary ==========="));
	UE_LOG(LogNL, Log, TEXT("  ... %d cues in always loaded list"), GCM->AlwaysLoadedCues.Num());
	UE_LOG(LogNL, Log, TEXT("  ... %d cues in preloaded list"), GCM->PreloadedCues.Num());
	UE_LOG(LogNL, Log, TEXT("  ... %d cues loaded on demand"), NumMissingCuesLoaded);
	UE_LOG(LogNL, Log, TEXT("  ... %d cues in total"), GCM->AlwaysLoadedCues.Num() + GCM->PreloadedCues.Num() + NumMissingCuesLoaded);
}

void UNLGameplayCueManager::OnGameplayTagLoaded(const FGameplayTag& Tag)
{
		...
		TGraphTask<FGameplayCueTagThreadSynchronizeGraphTask>::CreateTask().ConstructAndDispatchWhenReady([]()
			{
				if (GIsRunning)
				{
					if (UNLGameplayCueManager* StrongThis = Get())
					{
...
void UNLGameplayCueManager::HandlePostGarbageCollect()
{
...
void UNLGameplayCueManager::ProcessLoadedTags()
{
	...
	if (GIsRunning)
	{
		...
		{
			UE_LOG(LogNL, Warning, TEXT("UNLGameplayCueManager::OnGameplayTagLoaded processed loaded tag(s) but RuntimeGameplayCueObjectLibrary.CueSet was null. Skipping processing."));
		}
	}
}

void UNLGameplayCueManager::ProcessTagToPreload(const FGameplayTag& Tag, UObject* OwningObject)
{
	switch (NLGameplayCueManagerCvars::LoadMode)
	{
	case ENLEditorLoadMode::LoadUpfront:
...
void UNLGameplayCueManager::OnPreloadCueComplete(FSoftObjectPath Path, TWeakObjectPtr<UObject> OwningObject, bool bAlwaysLoadedCue)
{
...
void UNLGameplayCueManager::RegisterPreloadedCue(UClass* LoadedGameplayCueClass, UObject* OwningObject)
{
...
void UNLGameplayCueManager::HandlePostLoadMap(UWorld* NewWorld)
{
	if (RuntimeGameplayCueObjectLibrary.CueSet)
	{
...
void UNLGameplayCueManager::UpdateDelayLoadDelegateListeners()
{
...
bool UNLGameplayCueManager::ShouldDelayLoadGameplayCues() const
{
...
void UNLGameplayCueManager::RefreshGameplayCuePrimaryAsset()
{
