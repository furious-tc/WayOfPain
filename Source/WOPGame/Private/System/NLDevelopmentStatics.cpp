...
#include "System/NLDevelopmentStatics.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(NLDevelopmentStatics)

//@TODO: Actually want to take a lambda and run on every authority world most of the time...
UWorld* UNLDevelopmentStatics::FindPlayInEditorAuthorityWorld()
{
	check(GEngine);
...
TArray<FAssetData> UNLDevelopmentStatics::GetAllBlueprints()
{
...
UClass* UNLDevelopmentStatics::FindBlueprintClass(const FString& TargetNameRaw, UClass* DesiredBaseClass)
{
	FString TargetName = TargetNameRaw;
	TargetName.RemoveFromEnd(TEXT("_C"), ESearchCase::CaseSensitive);

	TArray<FAssetData> BlueprintList = UNLDevelopmentStatics::GetAllBlueprints();
...
UClass* UNLDevelopmentStatics::FindClassByShortName(const FString& SearchToken, UClass* DesiredBaseClass, bool bLogFailures)
{
