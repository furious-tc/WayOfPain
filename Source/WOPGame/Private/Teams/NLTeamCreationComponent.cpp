...
#include "Teams/NLTeamCreationComponent.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(NLTeamCreationComponent)

UNLTeamCreationComponent::UNLTeamCreationComponent(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	PublicTeamInfoClass = ANLTeamPublicInfo::StaticClass();
	PrivateTeamInfoClass = ANLTeamPrivateInfo::StaticClass();
}

#if WITH_EDITOR
EDataValidationResult UNLTeamCreationComponent::IsDataValid(FDataValidationContext& Context) const
{
...
void UNLTeamCreationComponent::BeginPlay()
{
	Super::BeginPlay();
...
void UNLTeamCreationComponent::OnGameReady()
{
#if WITH_SERVER_CODE
	if (HasAuthority())
	{
...
void UNLTeamCreationComponent::ServerCreateTeams()
{
...
void UNLTeamCreationComponent::ServerAssignPlayersToTeams()
{
	...
	for (APlayerState* PS : GameState->PlayerArray)
	{
		...
		{
			ServerChooseTeamForPlayer(NLPS);
		}
	}

	// Listen for new players logging in
	ANLGameMode* GameMode = Cast<ANLGameMode>(GameState->AuthorityGameMode);
	check(GameMode);

	GameMode->OnGameModePlayerInitialized.AddUObject(this, &ThisClass::OnPlayerInitialized);
}

void UNLTeamCreationComponent::ServerChooseTeamForPlayer(ANLPlayerState* PS)
{
...
void UNLTeamCreationComponent::OnPlayerInitialized(AGameModeBase* GameMode, AController* NewPlayer)
{
	check(NewPlayer);
	check(NewPlayer->PlayerState);
	if (ANLPlayerState* NLPS = Cast<ANLPlayerState>(NewPlayer->PlayerState))
	{
		ServerChooseTeamForPlayer(NLPS);
	}
}

void UNLTeamCreationComponent::ServerCreateTeam(int32 TeamId, UNLTeamDisplayAsset* DisplayAsset)
{
	check(HasAuthority());
	...
	ANLTeamPublicInfo* NewTeamPublicInfo = World->SpawnActor<ANLTeamPublicInfo>(PublicTeamInfoClass, SpawnInfo);
	...
	ANLTeamPrivateInfo* NewTeamPrivateInfo = World->SpawnActor<ANLTeamPrivateInfo>(PrivateTeamInfoClass, SpawnInfo);
...
int32 UNLTeamCreationComponent::GetLeastPopulatedTeamID() const
{
	...
	if (NumTeams > 0)
	{
		...
		for (APlayerState* PS : GameState->PlayerArray)
		{
			if (ANLPlayerState* NLPS = Cast<ANLPlayerState>(PS))
			{
				const int32 PlayerTeamID = NLPS->GetTeamId();

				if ((PlayerTeamID != INDEX_NONE) && !NLPS->IsInactive())	// do not count unassigned or disconnected players
				{
