...
#include UE_INLINE_GENERATED_CPP_BY_NAME(NLPawnComponent_CharacterParts)
...
FString FNLAppliedCharacterPartEntry::GetDebugString() const
{
...
void FNLCharacterPartList::PreReplicatedRemove(const TArrayView<int32> RemovedIndices, int32 FinalSize)
{
	bool bDestroyedAnyActors = false;
	for (int32 Index : RemovedIndices)
	{
		FNLAppliedCharacterPartEntry& Entry = Entries[Index];
...
void FNLCharacterPartList::PostReplicatedAdd(const TArrayView<int32> AddedIndices, int32 FinalSize)
{
...
void FNLCharacterPartList::PostReplicatedChange(const TArrayView<int32> ChangedIndices, int32 FinalSize)
{
...
FNLCharacterPartHandle FNLCharacterPartList::AddEntry(FNLCharacterPart NewPart)
{
	FNLCharacterPartHandle Result;
	Result.PartHandle = PartHandleCounter++;

	if (ensure(OwnerComponent && OwnerComponent->GetOwner() && OwnerComponent->GetOwner()->HasAuthority()))
	{
		FNLAppliedCharacterPartEntry& NewEntry = Entries.AddDefaulted_GetRef();
...
void FNLCharacterPartList::RemoveEntry(FNLCharacterPartHandle Handle)
{
	for (auto EntryIt = Entries.CreateIterator(); EntryIt; ++EntryIt)
	{
		FNLAppliedCharacterPartEntry& Entry = *EntryIt;
...
void FNLCharacterPartList::ClearAllEntries(bool bBroadcastChangeDelegate)
{
	bool bDestroyedAnyActors = false;
	for (FNLAppliedCharacterPartEntry& Entry : Entries)
	{
		bDestroyedAnyActors |= DestroyActorForEntry(Entry);
	}
	Entries.Reset();
...
FGameplayTagContainer FNLCharacterPartList::CollectCombinedTags() const
{
	FGameplayTagContainer Result;

	for (const FNLAppliedCharacterPartEntry& Entry : Entries)
	{
...
bool FNLCharacterPartList::SpawnActorForEntry(FNLAppliedCharacterPartEntry& Entry)
{
	bool bCreatedAnyActors = false;
...
bool FNLCharacterPartList::DestroyActorForEntry(FNLAppliedCharacterPartEntry& Entry)
{
	bool bDestroyedAnyActors = false;
...
UNLPawnComponent_CharacterParts::UNLPawnComponent_CharacterParts(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	SetIsReplicatedByDefault(true);
}

void UNLPawnComponent_CharacterParts::GetLifetimeReplicatedProps(TArray< FLifetimeProperty >& OutLifetimeProps) const
{
...
FNLCharacterPartHandle UNLPawnComponent_CharacterParts::AddCharacterPart(const FNLCharacterPart& NewPart)
{
	return CharacterPartList.AddEntry(NewPart);
}

void UNLPawnComponent_CharacterParts::RemoveCharacterPart(FNLCharacterPartHandle Handle)
{
	CharacterPartList.RemoveEntry(Handle);
}

void UNLPawnComponent_CharacterParts::RemoveAllCharacterParts()
{
	CharacterPartList.ClearAllEntries(/*bBroadcastChangeDelegate=*/ true);
}

void UNLPawnComponent_CharacterParts::BeginPlay()
{
	Super::BeginPlay();
}

void UNLPawnComponent_CharacterParts::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
void UNLPawnComponent_CharacterParts::OnRegister()
{
	Super::OnRegister();
...
TArray<AActor*> UNLPawnComponent_CharacterParts::GetCharacterPartActors() const
{
	TArray<AActor*> Result;
	Result.Reserve(CharacterPartList.Entries.Num());

	for (const FNLAppliedCharacterPartEntry& Entry : CharacterPartList.Entries)
	{
...
USkeletalMeshComponent* UNLPawnComponent_CharacterParts::GetParentMeshComponent() const
{
...
USceneComponent* UNLPawnComponent_CharacterParts::GetSceneComponentToAttachTo() const
{
...
FGameplayTagContainer UNLPawnComponent_CharacterParts::GetCombinedTags(FGameplayTag RequiredPrefix) const
{
...
void UNLPawnComponent_CharacterParts::BroadcastChanged()
{
